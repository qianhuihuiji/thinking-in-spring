...
public class BeanScopeDemo implements DisposableBean {
    ...
    public static void main(String[] args) {
        ...
        // 结论二：
        // 如果依赖注入是集合类型的对象，Singleton Bean 和 Prototype Bean 均会存在一个
        // Prototype Bean 有别于其他地方的依赖注入 Prototype Bean

        // 结论三：
        // 无论是 Singleton 还是 Prototype Bean 均会执行初始化方法回调
        // 不过仅 Prototype Bean 会执行销毁方法回调
    ...
    private static void scopedBeansByLookup(AnnotationConfigApplicationContext applicationContext) {
        for (int i = 0;i < 3;i++) {
            ...
            // prototypeUser s是每次依赖查找都生成了新的 Bean 对象
    ...
    public void destroy() throws Exception {
        ...
        for (Map.Entry<String, User> entry : this.users.entrySet()) {
            ...
            if (beanDefinition.isPrototype()) { // 如果当前 Bean 是Prototype scope
